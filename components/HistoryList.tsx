...
const HistoryList: React.FC<Props> = ({ history, onDelete, onLoad }) => {
  if (history.length === 0) {
    return (
      <div className="p-6 text-center text-slate-500 text-xs italic border-t border-slate-800">
    ...
    <div className="border-t border-slate-800 p-4">
      ...
      <div className="space-y-3">
        {history.slice().reverse().map((chat) => (
          <div key={chat.id} className="bg-slate-900 border border-slate-800 rounded-lg p-3 hover:border-slate-700 transition-colors group">
            <div className="flex justify-between items-start mb-2">
                <h4 className="font-bold text-xs text-white truncate max-w-[140px]" title={chat.scenarioName}>
                    {chat.scenarioName}
                </h4>
